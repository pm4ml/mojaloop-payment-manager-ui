...
class CSRExchange {
  constructor() {
    ...
    // sent CSR Items
    this.testClientHeader = Selector('div.certificate-card__header__details__primary').withText(
      'test_client'
    );
    ...
    this.csrPass = Selector('div.certificate-validation__modal__item-message').withText(
      'CSR passed verification'
    );
    this.csrSign = Selector('div.certificate-validation__modal__item-message').withText(
      'CSR has a valid Signature Algorithm : sha512WithRSAEncryption'
    );
    this.csrKey = Selector('div.certificate-validation__modal__item-message').withText(
      'CSR has a valid Public Key length of 4096'
    );
    this.csrName = Selector('div.certificate-validation__modal__item-message').withText(
